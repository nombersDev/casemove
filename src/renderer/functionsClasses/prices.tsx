...
import { Prices, Settings } from "renderer/interfaces/store";
...
export class ConvertPrices {
    ...
    getPrice(itemRow) {
        return this.prices.prices[this._getName(itemRow)]?.[this.settingsData.source.title] * this.settingsData.currencyPrice[this.settingsData.currency]
...
async function dispatchRequested(dispatch: Function, rowsToGet: Array<ItemRow>) {
...
export class RequestPrices extends ConvertPrices {
    ...
    _checkRequested(itemRow: ItemRow): boolean {
        return this.prices.productsRequested.includes(this._getName(itemRow)) == false
    }

    handleRequested(itemRow: ItemRow): void {
        if (this.getPrice(itemRow) == undefined && this._checkRequested(itemRow)) {
